...
static void write_ppm_ia4_tiled(const char* out_path, const unsigned char* src, u32 width, u32 height) {
    unsigned char* out;
    const unsigned char* cursor;
    u32 tile_x;
    u32 tile_y;
    ...
    for (tile_y = 0; tile_y < height; tile_y += 4) {
        for (tile_x = 0; tile_x < width; tile_x += 8) {
            for (y = 0; y < 4; y++) {
                for (x = 0; x < 8; x++) {
                    unsigned char b = *cursor++;
                    unsigned char intensity = (b >> 4) * 17;
                    u32 px = tile_x + x;
                    u32 py = tile_y + y;
...
int TPLExportFirstIA4(const char* out_path, const void* data, u32 size) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (format == 2 && dataOffset < size) {
            printf("exporting descriptor %u IA4 width=%u height=%u dataOffset=%u\n", i, width, height, dataOffset);
            write_ppm_ia4_tiled(out_path, p + dataOffset, width, height);
            return 1;
        }
    }

    printf("no IA4 texture found\n");
    return 0;
}
...
int TPLDecodeFirstCMPRToRGB(const void* data, u32 size, unsigned char** out_pixels, int* out_width, int* out_height) {
    ...
    if (!data || size < 12 || !out_pixels || !out_width || !out_height) {
        return 0;
    }
    ...
    *out_height = 0;
    ...
    for (i = 0; i < count; i++) {
        ...
        *out_height = height;
        ...
        printf("decoded first CMPR to RGB width=%u height=%u\n", width, height);
        return 1;
    }
